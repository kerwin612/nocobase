...
import { XlsxExporter } from '@nocobase/plugin-action-export';
...
export function findParentElementWithClass(options: {
  ...
  while (current) {
    ...
    const hasInclude = includeClass.length === 0 || includeClass.every((c) => current.classList.contains(c));
    const hasExclude = excludeClass.some((c) => current.classList.contains(c));
