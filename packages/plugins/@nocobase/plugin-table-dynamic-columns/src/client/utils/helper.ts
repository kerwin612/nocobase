...
export const addDynamicColumnHeaders = (table: HTMLTableElement, colNumber: number, dynamicColumns: any[]) => {
  ...
  const configTh = ths ? ths[colNumber === -1 ? ths.length - 1 : colNumber] : null;
  if (!configTh) {
  ...
  if (colgroup) {
    ...
    const configCol = cols ? cols[colNumber === -1 ? cols.length - 1 : colNumber] : null;
...
export const updateDynamicColumnRows = (
  ...
  rows.forEach((row) => {
    ...
    const configTd = tds ? tds[colNumber === -1 ? tds.length - 1 : colNumber] : null;
    if (!configTd) return;
    // 移除旧的动态列
    tds.forEach((td) => {
