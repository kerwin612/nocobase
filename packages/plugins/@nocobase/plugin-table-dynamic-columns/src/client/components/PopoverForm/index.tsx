...
// onApply 是表单应用的回调函数
// onClose 是表单关闭的回调函数
export const PopoverForm = ({ formMetaData, values, onApply, onClose }) => {
  ...
  return (
    <Form form={form} initialValues={values}>
      {formMetaData.map((item) => {
        if (!item.type || item.type === 'string') {
          return (
            ...
            >
              <Input autoComplete="off" placeholder={item.placeholder} />
            </Form.Item>
          );
          ...
          return (
            <Form.Item
              ...
              rules={[{ required: item.required, message: `请输入${item.label}` }]}
            >
              <InputNumber autoComplete="off" placeholder={item.placeholder} />
            </Form.Item>
          );
          ...
          return (
            <Form.Item
              ...
              rules={[{ required: item.required, message: `请输入${item.label}` }]}
            >
              <DatePicker placeholder={item.placeholder} showTime />
            </Form.Item>
          );
        } else {
          return null;
      ...
      <Space style={{ width: '100%', justifyContent: 'flex-end' }}>
        <Button onClick={onClose}>关闭 - 动态列</Button>
        <Button
          type="primary"
          onClick={() => {
            form
