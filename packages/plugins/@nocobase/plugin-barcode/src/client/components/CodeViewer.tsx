...
export const BarcodeViewer: FC<BarcodeViewerProps> = ({
  ...
  try {
    ...
    try {
      codeContent = executeValueGetter(valueGetter, objectValue, fieldValue);
    } catch (e) {
      console.error(t('Failed to evaluate code valueGetter'), e);
      throw new Error(`${t('Code valueGetter error')}: ${e.message}`);
    }

    if (!codeContent) {
      throw new Error(t('Code content is empty'));
    }
