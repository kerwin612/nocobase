...
export const SchemaSettingsEnableBarcode = () => {
  const { t } = useTranslation(NAMESPACE);
  const { collectionField, field, fieldSchema, isActionField, isInputField } = useSettingsContext();
  ...
  return (
    <SchemaSettingsSwitchItem
      ...
      hidden={!collectionField} // table operation column can't enable barcode
      onChange={(v) => {
        if (!v) {
...
export const SchemaSettingsCustomBarcode = () => {
  ...
  return (
    <SchemaSettingsModalItem
      ...
      width={'60vw'}
      components={{ Switch }}
      hidden={!(enableBarcode && isFieldReadPretty)}
      schema={
        {
          type: 'object',
          properties: {
