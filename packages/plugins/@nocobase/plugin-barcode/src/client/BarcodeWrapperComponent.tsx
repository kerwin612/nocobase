...
export const BarcodeWrapperComponent: FC = observer(
  () => {
    ...
    try {
      codeContent = executeValueGetter(codeConfig.valueGetter, record?.data, field?.['value']);

      if (!codeContent) {
        codeContent = String(field?.['value'] ?? '');
      }
    } catch (e) {
    ...
    const component = useBarcodeAction({
      component: <span>{codeContent ?? 'Empty content'}</span>,
      codeConfig,
      value: codeContent,
