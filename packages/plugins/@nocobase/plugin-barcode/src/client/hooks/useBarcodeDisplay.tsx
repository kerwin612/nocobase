...
export const useBarcodeDisplay = ({
  ...
  try {
    codeContent = executeValueGetter(codeConfig.valueGetter, objectValue, fieldValue);

    if (!codeContent && defaultContent) {
      codeContent = defaultContent;
    }

    if (!codeContent) {
      console.error('Barcode content is empty');
    }
  } catch (e) {
    ...
    if (defaultContent) {
      ...
      console.error(`Code valueGetter error: ${e.message}`);
    }
  }
